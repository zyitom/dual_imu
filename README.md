...
调度。predictor 历史只由主线程推进并发布不可变双缓冲 snapshot，TIM5 ISR 只做固定
复杂度的因果预测，不再遍历共享历史或依赖长临界区。完整 ARM
Debug/Release/RelWithDebInfo 固件可链接，原有十二组加 motion guard、HI91 capture、
HI91 static analysis 共十五组普通 host tests 通过，十四个 C 测试目标的 ASan/UBSan
回归也通过。当前 `93ec...` Release 已刷入并独立 `verifybin` 成功；下述冷启动及 60 s
静置数据属于此前 `852d...` 镜像的历史短测。诊断模式 1 s 窗的代表计数约为
`1608-1612/1997-2000/1610-1612/3219-3223 Hz`；名义配置依次为
...
`state.mekf_quaternion/mekf_euler_rad`。此前 `852d...` 镜像的短时实板结果证明目标
频率、FIFO 数据链、时钟映射、状态字和 1600 Hz 发布调度已经闭合；当前 `93ec...`
镜像的 CDC 冷启动复测和完整 2 h 连续长跑仍须从零完成，不能继承旧文件的结论。
...
| `BM0_PWMD14` | PD14 | GPIO push-pull，固定低 | IMU 加热使能安全关闭，不启动 PWM |
| `BM0_PWM` | PD15 | GPIO push-pull，固定低 | IMU 加热使能安全关闭，不启动 PWM |
...
sequence、丢弃本批并锁存 `GYRO_TIMING`，停止该 lane 的后续 gyro DMA。

锁存后先 fail-close，20 ms 后由主线程尝试只重建 BMI gyro 边界：原子关闭并清理
TIM5_CH2/capture 队列，保持 gyro NORMAL，不做 suspend/normal power cycle；FIFO 执行
BYPASS，等待真实 DRDY seed，再切 STREAM，并回读 gyro chip ID、量程/ODR/电源、INT3
DRDY 映射和 gyro FIFO。SPI1 暂忙只延迟 1 ms 再试，不计作失败；真实同步、总线或配置
失败才退避 1 s。边界成功后仍保持 gyro 输出无效，直到首个 recovery 后可信 batch 发布，
不沿用故障前旧 gyro，也不按固定 500 us 伪造时间戳。该恢复只覆盖 BMI gyro
FIFO/capture，不代表 accel、ICM 或任意总线故障都能自动恢复。
...
不能改成 82。此前 `852d...` 的 1600 Hz、82-byte 满负载板测下，OTG_FS IRQ 优先级 0 时 TIM5
...
仍在 USER CODE 区。随后重新运行十五组普通 host tests、十四个 C sanitizer 目标、三个
ARM build 和短时 USB 板测。
...
配置现在分别只由 `.ioc`/CubeMX 生成，SPI 初始化后只保留寄存器断言。当次再生成后的
十二组主机测试和 Debug/Release/RelWithDebInfo 均通过；“USB 未枚举”是当次历史记录。
当前稳定设备名为
`/dev/serial/by-id/usb-STMicroelectronics_STM32_Virtual_ComPort_366437643533-if00`
和 `/dev/serial/by-id/usb-SEGGER_J-Link_000069655208-if00`。内核分配的短设备名会随枚举
顺序交换，脚本和验收记录不得把它当作稳定身份。当前 `93ec...` 镜像已刷写并独立验证；
其 USB 冷启动和静置复测尚待 CDC 重新枚举。第 8 节中的冷启动和 60 s 数值均是此前
`852d...` 镜像的历史回归证据。

### 3.5 加热引脚的失效安全边界

生产代码没有 IMU heater/PWM 启动，也没有温度闭环。复位最初到 `HAL_Init()` 返回前，
PD14/PD15 的电平仍依赖板级硬件下拉，软件无法替代这段硬件安全设计。`HAL_Init()` 后、
进入 `SystemClock_Config()` 前，固件先开 GPIOD 时钟并把 PD14/PD15 配为 push-pull low；
随后 `MX_GPIO_Init()` 再次写低并重复相同 GPIO 配置。这样即使时钟配置进入
`Error_Handler()`，软件接管后的加热使能也保持关闭。
...
- `sequence`、`selected_source`、`integrity_flags`、`predicted`、`degraded`、
  `deadline_miss`、`euler_singular`、逐帧 accel/gyro saturation snapshot 和 `valid`。

这里的 `valid` 只证明数值、时间链和样本覆盖满足接口约束，不表示逐台标定已加载、
ZARU 已收敛或姿态精度已经达标。
`dual_imu_get_attitude()` 的 latest copy 使用奇偶 generation seqlock；即使读取被中断并
跨过两次 625 us 发布、双缓冲 index 发生 ABA，也会重试而不会接受跨代拼接的数据。
...
标志。主线程每个选中 gyro 样本推进 replay cache，并把最近两个因果端点导出到不可变
snapshot；TIM5 ISR 只读取已发布 snapshot，复杂度不随历史长度变化。它不会把 400 Hz
的重力观测伪装成 1600 Hz 新观测。
...
输入。统计先预热 40 窗；每个已 seed 且有效的 lane 使用当前 MEKF bias，要求
`norm(stationary_gyro_mean - current_mekf_bias) <= 0.5 dps`，不是逐轴比较，也没有
relaxed bias bootstrap。可观的横滚/俯仰 bias 可先由重力更新估计；物理一致的持续旋转
回放必须被静止门拒绝，绕重力轴的恒定慢转仍是不可观歧义。实测 temporal gyro 总 RMS
门按 lane 定为 BMI `0.8 dps`、ICM `0.25 dps`，accel 总 RMS 门为 `0.10 m/s^2`。
ZARU 对当前 calibration pipeline 输出的 target 逐轴限制为 `+/-0.5 dps`，bias
correction 三维向量的 slew 上限为 `0.1 dps/s`。`lane_calibrated` 只表示本次上电的
易失 bias trim 收敛，不表示 scale、安装或温度标定完成；后续逐台标定仍应覆盖 accel、
gyro 零偏及温度项，在此之前不承诺 yaw 漂移指标。

当前用户称设备“平放”时，两颗加计在公共坐标中的代表值约为
`[-7.05, +1.9, +6.5] m/s^2`，输出约 `roll=-164 deg, pitch=-46 deg`，两者按当前 FRD/ZYX
定义与同一重力方向数学一致。这说明算法没有凭空把三个轴算歪，但不能仅靠一个静止姿态
判断是板体放置、装配方向还是标定参数导致。绝不能用这一帧强写安装矩阵，把显示归零
冒充标定；必须采集六面静置数据求 accel bias/scale/非正交与相对安装，再做绕 X/Y/Z
正方向转动确认符号、Hamilton 四元数和 ZYX 欧拉约定。

厂商资料本身存在不能忽略的旋转矛盾：用户手册和厂商 ArduPilot `hwdef.dat` 都写
`ROTATION_PITCH_180`，但从厂商公开 PX4 V1.16 ELF 的 CROMFS 中解出的
`rc.board_sensors` 对 BMI088 和 ICM45686 都传入 `-R 4`；PX4 v1.16 的 `Rotation` 枚举中
`4` 是 `ROTATION_YAW_180`，`ROTATION_PITCH_180` 是 `12`。当前代码暂按手册和
ArduPilot 的两份一致证据保留 pitch-180。这个离散旋转差异只会换轴/换符号，无法把一个
真实平放时应接近单一 Z 轴的重力解释成当前三轴都有大分量，因此不能据此盲改固件；最终
选择必须由同一实物的六面原始数据和三轴正转共同裁决。

### 5.2 USB HI91 兼容帧

USB CDC 默认输出固定 82-byte、小端二进制帧，布局与用户给出的 HI91 `0x91` 数据包
一致；`main_status` 的公开位也按下述 HI91 语义输出，保留位仍属于本项目私有诊断。
...
| 6 | 1 | tag | `0x91` |
| 7 | 2 | `main_status` | HI91 公开状态位及项目私有保留位 |
| 9 | 1 | temperature | 编码时选中 lane 最新数值，deg C，四舍五入并限幅到 int8；帧内无 valid/stale 位 |
| 10 | 4 | air pressure | 无传感器，固定 `+0.0f` Pa |
| 14 | 4 | system time | TIM5 输出 epoch/1000 的低 32 bit，ms |
| 18 | 12 | acceleration XYZ | 正常为最近因果 400 Hz fused specific force；invalid 时可短时保留最近可信值，g |
| 30 | 12 | angular rate XYZ | 正常为当期去偏角速度；invalid 时可短时保留最近可信值，deg/s |
| 42 | 12 | magnetic field XYZ | 无传感器，固定 `+0.0f` uT |
| 54 | 12 | roll/pitch/yaw | 正常为当期 ZYX Euler；invalid 时可短时保留最近可信值，deg |
| 66 | 16 | quaternion W/X/Y/Z | 正常为当期 body-to-NED；invalid 时可短时保留最近可信值 |

CRC 为 CRC-16/CCITT，初值 0、多项式 `0x1021`，依次覆盖 frame bytes `0..3` 和
`6..81`，跳过 bytes `4..5` 的 CRC 字段。给定样例结果为 `0xBB14`，线上按小端发送
`14 BB`。

`main_status` 的公开位按 HI91 状态字语义输出。注意 `WB_CONV`、`ATT_CONV`
都是反向质量位，`1` 表示尚未收敛，而不是已经收敛：

| bit | 名称 | 本固件 `1` 的含义 |
|---:|---|---|
| 3 | `WB_CONV` | 当前选中 gyro lane 的在线零偏尚未收敛 |
| 4 | `MAG_DIST` | 磁场可信度异常；当前为 6 轴模式，固定为 0 |
| 5 | `ACC_SAT` | 当前帧时刻或此前不足 2 s 内任一加速度计发生量程饱和 |
| 6 | `GYR_SAT` | 当前帧时刻或此前不足 2 s 内任一陀螺仪发生量程饱和 |
| 7 | `ATT_CONV` | 当前 fast frame 无效、姿态未收敛、重力辅助连续超时、冲击后姿态仍在重捕获、转动不可观、航向连续性已经丢失，或 Euler 接近奇异区 |
| 9 | `STATIC` | 内部静止状态机已确认静止 |
| 10 | `MAG_AIDING` | 9 轴磁辅助使能；当前为 6 轴模式，固定为 0 |
| 11 | `UTC_UNSYNC` | `system_time` 尚未 UTC 同步；当前固定为 1。当前时间字段约 49.7 天回卷，并非 24 h 本地时钟 |
| 12 | `SOUT_PULSE` | 本帧对应一次真实 SOUT 脉冲；当前未输出脉冲，固定为 0 |

bit 0--2、8、13--15 是协议保留位。固件目前只把它们作为项目内诊断使用：bit 0--2
分别是 attitude/accel/gyro valid，bit 8 是 sticky `heading_continuity_lost`，bit 13/14
是 BMI/ICM fault，bit 15 是本 DTR 会话 stream drop。外部 HI91 客户端必须屏蔽这些
保留位，不能依赖其
取值；项目自带的 `tools/hi91_capture.py` 可解析它们用于板级验收。公开位之外的定义
以后可能变化。

`heading_continuity_lost` 表示姿态链已经经历无法由六轴系统事后证明的转角：包括两个 gyro
同时不可观、真实量程冲击使共同 g-sensitivity 与真实转动不可区分，或 accel inhibit 期间
两路 gyro 差分 NIS 明确冲突。重力只能重新观测 roll/pitch，不能补回这段 yaw，因此该位
本次上电 sticky，不会因为 roll/pitch 重捕获完成而清除。
从该事件时间戳起，公开 `ATT_CONV` 也保持 1；即使 roll/pitch 已由重力重新收敛，标准
客户端也不会把仍不可证明的 yaw 误判为三轴姿态精度正常。`ATT_CONV` 对任何 invalid
姿态帧同样强制置 1，所以忽略私有 valid 位的标准客户端不会把 held/零填姿态误判为
“精度正常”。重力辅助连续 800 个 400 Hz 窗未成功时，内部姿态收敛状态重新撤销，
`ATT_CONV` 同样置 1。

`ACC_SAT`/`GYR_SAT` 使用传感器样本时间而不是 USB 编码时刻：事件当帧置 1，
`event + 1,999,999 us` 仍为 1，在 `event + 2,000,000 us` 清 0。主线程把三段按时间
排序、合并相邻/重叠区间的归一化历史以不可变双缓冲 snapshot 发布给 TIM5 ISR；每个
fast frame 在自身 scheduled epoch 固化两个 saturation 布尔值。USB 编码时对帧内值和
稍后才由主线程处理到的 epoch 历史取 OR，因此长 backlog 仍保持因果，事件前旧帧不会
被未来事件污染。关闭并重新打开 DTR 也不会清除设备侧的 2 s 历史。
...
权威时间语义。accel 的信息率仍是 400 Hz，正常会在四个 fast frame 中重复；超过
7.5 ms 或无效时撤销 accel-valid。为避免短暂冲击/切换把无效数值误显示成三轴瞬时归零，
attitude/gyro/accel 在最多 20 ms 内可携带最近可信 payload，但 bit0/1/2 仍按当前 epoch
清零且是唯一权威有效性；超过 20 ms 才零填。项目内消费方不得忽略保留位 0--2 而把
held payload 当作实时测量。温度字段只携带编码时选中 lane 的最新数值，HI91 帧没有
温度 valid/stale 状态；诊断和温补逻辑内部另行保留温度时间戳及有效性，客户端不能从
该 int8 字段单独证明新鲜度；从未取得有效温度也按 stale 处理。bias/attitude
convergence、stationary、fault 等慢状态取
编码时的最新值，pressure/magnetic field 因无硬件来源始终固定为 `+0.0f`。
...
- 当前 motion guard 对 gyro 单 lane 连续 clipping 去抖 3 ms；两 gyro 在 3 ms 内共同
  clipping 或任一 accel clipping 视为冲击证据，gyro 硬锁存抑制 20 ms、加计修正抑制
  100 ms，饱和样本本身始终拒绝积分。BMI gyro FIFO/capture 有上述受控边界恢复。
- 温度有效性、因果、2 s 新鲜度、范围 clamp、变化率门和输出坐标 correction slew 已
  实现；accel/gyro 各自维护温度门历史，避免 BMI 两条独立 FIFO 的消费顺序制造伪
  noncausal。production build 仍在编译期剔除高阶温补；运行期全配置巡检、BMI
  accel/ICM/广义总线恢复、IWDG 和持久标定存储仍属于后续保护工作。

## 7. 几何与标定

两颗加计封装中心距按当前 STEP 数据为 7.1 mm。当前基线依手册和厂商 ArduPilot
`hwdef.dat`，把两颗器件按 `ROTATION_PITCH_180` 转入 FRD；上节记录的厂商 PX4
`-R 4` 矛盾仍须六面实测裁决。在当前基线下使用：
...
标定管线固定为
`calibrated = correction_matrix * (nominal_body - bias(T))`。3x3 matrix 同时承载逐台
scale/非正交和测得的残余安装旋转；bias 多项式为
`c0 + c1*dT + c2*dT^2 + c3*dT^3`。静态 `c0` 和 correction matrix 始终应用，即使高阶
温补关闭；关闭时 `dT=0`，所以 `c1..c3` 不参与计算。

ARM target 显式固定 `IMU_CALIBRATION_PRODUCTION_BUILD=1`、
`IMU_CALIBRATION_HIGH_ORDER_TEMPERATURE_COMPENSATION_BUILD_ENABLE=0` 和
`IMU_CALIBRATION_G_SENSITIVITY_BUILD_ENABLE=0`。production 与任一资格功能同时为 `1`
会在预处理阶段报错，CMake 也拒绝任何非 production/high-order-off/g-sensitivity-off 的
固件配置。高阶温补宏为 `0` 时 runtime enable、温度历史、高阶选择和 slew 状态均不进入
最终对象，任何 enable=true 请求都会被拒绝；静态 `c0` 与 correction matrix 仍按原运算
顺序应用。

隔离的 host qualification build 才允许 high-order=`1`。其中 accel/gyro 分别维护温度
门和 applied correction；目标为 `correction_matrix*bias(T)`，实际输出为
`correction_matrix*raw-applied_correction`。首次从 `matrix*c0` 开始，无效、过期、非因果、
变化率拒绝、clamp、disable 和恢复只改变 target，再按三维向量范数限速。默认 accel
上限为 `0.0980665 m/s^3`，gyro 为 `0.00174532925 rad/s^2`，单次只使用最多 10 ms 的
样本间隔，因此长断流不能换来一次大修正。两种宏配置、异常往返、双流交错和长 gap 都有
普通及 ASan/UBSan 主机回归。

这仍不是启用许可：当前没有逐台热箱系数、带 CRC/版本的掉电持久化或实物热循环验收，
生产路径也没有 enable 调用。这些全是被动数值修正，不会启动 heater、PWM 或温度闭环；
PD14/PD15 的上电失效安全边界见 3.5 节。

陀螺静态 g 敏感度的二阶段修正只存在于隔离的 qualification build：
`gyro_out = gyro_temp_matrix - K_ga * accel_body`，其中 `accel_body` 是同一颗 IMU 经轴向和
校准后的 body/output-frame specific force，`K_ga` 的单位为
`(rad/s)/(m/s^2)`。每颗 IMU 有独立的因果 accel 历史；只接受不晚于 gyro epoch、未饱和、
时间可信且足够新的同源样本，gap、invalid、未来样本和 saturation 都 fail closed。加载
calibration 不会启用该功能，runtime enable 还要求自定义 calibration 且只能在首个流样本
前设置。production 中该历史、矩阵乘法和 enable 状态均编译剔除。

当前两颗器件的 `K_ga` 都是零矩阵，固件从未启用修正。现有单一平放静置序列只能估计该
姿态下的 gyro bias/noise，无法从一个固定 specific-force 向量辨识 3x3 矩阵。至少需要
重复多姿态静置数据来分离常值 bias 和各列系数；若要证明冲击/振动下有效，还需要带真值
角速度的 rate table，以及 shaker 或 centrifuge 的受控加速度幅频/方向数据。完成参数拟合、
留出集验证和动态恢复测试前，不宣称这条资格代码提高了实物姿态精度。gyro bias 截距必须
与 `K_ga` 联合拟合，不能把已经包含某一静置重力方向响应的“平放零偏”直接当作截距，否则
会重复补偿。这个静态线性模型也不能重建量程 clipping 已经丢失的转角，不能替代冲击后的
invalid 标记和姿态重捕获。
...
| 项目 | 当前状态 |
|---|---|
| MEKF、预积分、FIFO、clock-sync、HI91、USB stream、motion guard、capture 和 static analysis 主机测试 | 十五组普通 host tests 全部通过；十四个 C 目标的 ASan/UBSan 全部通过 |
| Debug/Release/RelWithDebInfo ARM 固件 | 三种 clean-first 构建无 warning；Release BIN `148384 B`，SHA-256 `93ec06e7684d3c667a33046debf3baa8c265002631e2e9e5aab4020785afb54d` |
...
| 内部静止/ZARU | 此前 `852d...` 冷启动 `STATIC=4928 ms`、`WB_CONV` 最终清除 `6133 ms`，两 lane 易失 bias trim 收敛；`93ec...` 待复测 |
| lane 故障切换 | 主机 yaw/tilt alignment 回归通过；调试停机注入后 BMI fail-close 并切到 ICM |
| 冲击/clipping 保护 | 3 ms gyro 去抖、3 ms 双 lane 共同冲击、20 ms 锁存抑制、100 ms accel inhibit；主机合成测试通过，实物剧烈冲击仍待用户实际执行 |
| BMI gyro 边界恢复 | 20 ms 后受控重建，暂忙 1 ms 重试、真实失败 1 s 退避、完整 gyro 回读及首批新数据门控；代码/构建通过，故障注入板测待做 |
| 400 Hz MEKF | 实测 399-401 Hz |
| 1600 Hz fast quaternion/Euler | 此前 `852d...` 60 s 收到 96006 帧，`1599.928 Hz`；首尾各 5 s 均值差 roll/pitch/yaw 为 `-0.019/-0.004/+0.024 deg` |
| TIM5_CH3 625 us 发布调度 | 正常时 miss/drop=0，发布延迟峰值小于 20 us |
| USB HI91 82-byte/1600 Hz | 此前 `852d...` 冷启动首帧 `518 ms`，`ATT_CONV` 清除 `527 ms`，ICM/BMI fault 最终清除 `738/2928 ms`；60 s CRC/resync/drop/nonfinite/valid-dropout/valid 零 triplet 均为 0 |
| 2026-07-15 CubeMX 再生成 | 当时软件审计、十二组测试、三个 ARM build 和 DMA map 通过；当前工作树为十五组 |
| 时钟链 | 启动 timeline reset=1（预期）；稳态 clock reset/discontinuity/causal error=0 |
| 逐台标定、`K_ga` 与四条绝对 pipeline delay | 未完成；当前 calibration 未加载、`K_ga` 为零且编译关闭，delay 宏为 0 |
| 最终 2 h 数据完整性 | `93ec...` 完整采集尚待 CDC 重新枚举后从零开始；旧 `852d...` 的 1156 s 文件已废弃且不得拼接 |

传感器表中的 ODR 是寄存器名义值，1 s 状态行只是带量化的代表计数，不是精确 ODR
测量。affine clock model 用来吸收 sensor clock 与 TIM5 的斜率差，不能把短窗计数
反写成寄存器配置。此前 `852d...` Release 已证明短时持续 400 Hz MEKF、`1599.928 Hz`
有效 fast 输出、内部 ZARU、故障 fail-close 和 USB 满负载输出。60 s 的 96006 帧中完整性相关计数
全部为 0；这只能支持静置短时数据链结论，不能扩展为逐台姿态精度、动态冲击恢复或 2 h
长期可靠性。剩余工作包括完成 `93ec...` 的冷启动复测和全新 2 h 采集、由用户实际执行剧烈冲击、逐台标定、
pipeline delay 标定和受控 BMI recovery 故障注入。
...
4. 静置连续 2 h：上述 error/drop 保持 0，BMI 24-bit、ICM 16-bit 和 TIM5 32-bit 的
   正常回卷成功 unwrap，映射时间严格单调。`93ec...` 采集尚未开始，旧文件不能继承。
...
7. USB 主机置 DTR 后连续检查 82-byte 对齐、CRC、1600 Hz 帧率、timestamp 单调性、
   pressure/mag 零填，以及 saturation/convergence/private-drop 状态位；deadline 通过
   private valid 和内部发布诊断验收。此前 `852d...` 的冷启动及 60 s 单会话短测已满足；
   当前 `93ec...` 仍待复测，
   实物冲击状态位仍须由用户实际施加后验收。
...
1. CDC 重新枚举后先完成 `93ec...` 冷启动短测，再从零运行完整 2 h 连续采集并保存 CSV
   与采集摘要；旧 `852d...` 的 1156 s 文件不得拼接或作为最终验收。
2. 受控注入一次 BMI capture fault，验证自动恢复；不得用调试器长时间停核制造自然
   运行结论。
3. 加载并验证逐台 accel/gyro/temperature 标定；用多姿态及受控 rate-table/shaker 或
   centrifuge 数据辨识和独立验证 `K_ga`，不得从当前单一平放静置记录拟合。
4. 标定四条 pipeline delay；当前宏全部为 0，不宣称绝对动态相位。
5. 做受控六面、三轴正转及接近 Euler 奇异区测试，确认 FRD/Hamilton/ZYX 符号；不能
   用单姿态显示归零替代。
6. 由用户实际施加剧烈振动/冲击并同步采集 status、valid、零值和恢复时间；主机合成
   测试不能替代这一项。
7. 后续保护包括全配置 watchdog、其他 lane/总线恢复、IWDG 和掉电安全存储；不实现
   IMU 加热器、PWM 温控或温度闭环。
...
- [DM-FC01 vendor firmware and mechanical files](https://github.com/dmBots/dm-fc01)
- [PX4 v1.16 Rotation enum](https://github.com/PX4/PX4-Autopilot/blob/v1.16.0/src/lib/conversion/rotation.h)
- [PX4 current multi-EKF selector](https://github.com/PX4/PX4-Autopilot/blob/main/src/modules/ekf2/EKF2Selector.cpp)
- [ArduPilot EKF lanes overview](https://ardupilot.org/copter/docs/common-apm-navigation-extended-kalman-filter-overview.html)

...
> **自定义双 IMU 固件安全警告**
>
> 本手册后文关于 PX4/ArduPilot heater 参数和“默认启用加热器”的说明，只适用于对应的
> PX4/ArduPilot 固件，不适用于本仓库的 `dual_imu` 自定义固件。运行本项目固件时，禁止
> 执行第 6 节的 heater 启用命令、参数修改或温度闭环操作。
>
> 本项目固件在 `HAL_Init()` 后、system clock 配置前先将 PD14/PD15 写低并配置为
> push-pull output，随后 `MX_GPIO_Init()` 再次写低。当前最终 ELF 没有 PWM Start 调用，
> 也没有 heater 或温度闭环；高阶温度补偿只是默认锁止的数值校正，与加热器无关。
> 但是 MCU 上电/复位的最初阶段，PD14/PD15 在软件接管前仍为 Hi-Z。仓库内没有外部门极
> 下拉的原理图或实测证据，因此软件写低不能替代硬件 fail-off；涉及加热器安全时必须先
> 确认外部下拉或测量复位波形。
